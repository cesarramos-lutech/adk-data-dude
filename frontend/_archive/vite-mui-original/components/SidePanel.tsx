...
// Fallback when /list-apps fails or returns empty (e.g. playground backend uses folder name)
const DEFAULT_AGENT_FALLBACK: AgentOption[] = [
  { id: 'dashboard_agent', name: 'Dashboard Agent' },
];
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      ...
      try {
        ...
        const agentList: AgentOption[] = Array.isArray(response) && response.length > 0
          ? response.map((name: string) => ({ id: name, name }))
          : DEFAULT_AGENT_FALLBACK;
        setAgents(agentList);

        const firstAgentId = agentList[0]?.id;
        if (!firstAgentId) {
          setIsGlobalLoading(false);
          return;
        }
        ...
        if (urlApp && agentList.some(a => a.id === urlApp)) {
        ...
        // CASE B: Default Load (No URL params) -> Select First Agent & Create Session
          ...
          await runFullSessionFlow(firstAgentId);
        }
      } catch (err) {
        console.error("Initialization Flow Failed", err);
        setAgents(DEFAULT_AGENT_FALLBACK);
        setSelectedAgentId(DEFAULT_AGENT_FALLBACK[0].id);
        try {
          await runFullSessionFlow(DEFAULT_AGENT_FALLBACK[0].id);
        } catch {
          // Backend may be down; user can still select agent and retry
        }
      } finally {
        setIsGlobalLoading(false);
      }
